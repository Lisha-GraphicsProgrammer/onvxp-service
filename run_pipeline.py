...
# ============================================================
# VIDEO CLIP EVIDENCE SETTINGS
# ============================================================
CLIP_FPS_FALLBACK = 15  # used only if the source video reports 0 fps
CLIP_SECONDS_BEFORE = 4
CLIP_SECONDS_AFTER = 2.5
...
# ── Video clip evidence: rolling buffer of recent raw frames, plus the
# fps-derived frame counts needed to turn "N seconds before/after" into
# actual frame counts for this specific video. ──
_cap_fps = cv2.VideoCapture(video)
VIDEO_FPS = _cap_fps.get(cv2.CAP_PROP_FPS) or CLIP_FPS_FALLBACK
_cap_fps.release()
CLIP_FRAMES_BEFORE = int(VIDEO_FPS * CLIP_SECONDS_BEFORE)
CLIP_FRAMES_AFTER = int(VIDEO_FPS * CLIP_SECONDS_AFTER)
frame_buffer = collections.deque(maxlen=CLIP_FRAMES_BEFORE)
print(f"[INFO] Clip buffer: {CLIP_FRAMES_BEFORE} frames before, {CLIP_FRAMES_AFTER} frames after (fps={VIDEO_FPS:.1f})\n")


def save_incident_clip(incident_id, buffer_frames, video_path, start_frame_idx, frames_after, fps, width, height):
    """Writes buffer_frames (the 'before' window, raw frames already
    captured in the rolling buffer) plus frames_after more frames read
    forward from video_path starting right after start_frame_idx, into a
    single mp4. Opens a SECOND capture on the same file so the main
    tracking stream (a one-shot generator) is never touched or rewound."""
    clip_path = f"incidents/{incident_id}.mp4"
...
for frame_idx, result in enumerate(results):
    ...
    frame_buffer.append(frame_img.copy())
    ...
    for rule_idx, rule in enumerate(rules):
        ...
        if cooldown_key not in active_violations:
            ...
            clip_path = save_incident_clip(
                incident_id, frame_buffer, video, frame_idx,
                CLIP_FRAMES_AFTER, VIDEO_FPS, ORIG_W, ORIG_H
            )
    ...
    #
    # Counts everyone currently tracked inside the zone THIS frame in a
    # single pass (not per-person), compares against rule['count'], and
    # fires at most one incident per rule when the threshold is exceeded.
    # Cooldown is keyed per-rule (rule_idx, 'count') — one zone-level
    # violation, not one per excess person.
    # ============================================================
        ...
        for rule_idx, rule in enumerate(rules):
            ...
            if violation_occurred and cooldown_key not in active_violations:
                ...
                print(f"Frame {frame_idx}: person #{person_id} | rule[{rule_idx}] {rule_type} in {zone_name}"
                      ...
                      + f" → {incident_id} [SAVED] (clip: {clip_path})")
